...
zss=sc/ss;
cs=ss/po;
fprintf('\n THE COIL SPAN OF THE INDUCTION MOTOR=%.4f \n',cs);
evenno=input('\n IF THE CS VALUE IS EVEN , THAN TYPE 2 ELSE ANY OTHER NO= \n');
cs1=2;
if cs1==evenno
 alpha=(1/cs)*180;
...
Lg1=input('\n if lg is greater than 0.3 it must be taken as 0.0003 METERS = \n');
Dr=D-2*Lg1;
Sr=input('\n rotor slots= \n');
ysr=(pi*Dr)/Sr;
...
Tclb=Sr*Ib^2*rb*1e6;
fprintf('\n########################################################################## \n');
fprintf('\n OUTPUT OF THE ROTOR DESIGN OFINDUCTION MOTOR \n ');
fprintf('\n########################################################################## \n');
...
Wst=Vst*7.6*1e3;
Bst=(pi/2)*Bts; 
fprintf('\n THE FLUX DENSITY CORRESPONDING TO STATOR TEETH OF THE INDUCTION MOTOR=%.4f WB/M2 \n',Bst);
Lkg=input('\n corresponding to Bst loss per kg in w= \n');
ILst=Lkg*Wst;
%iron loss in stator core
Vsc=input('\n volume of stator core in METER CUBE(M3)= \n');
wsc=Vst*7.6*1e3;
Bsc=Bcs;
fprintf('\n THE FLUX DENSITY CORRESPONDING TO STATOR CORE OF THE INDUCTION MOTOR=%.4f WB/M2 \n',Bsc);
...
fprintf('\n########################################################################## \n');
fprintf('\n OUTPUT OF LOSS COMPONENT OF INDUCTION MOTOR \n');
fprintf('\n########################################################################## \n');
fprintf('\n THE IRON LOSS IN STATOR TEETH OF THE INDUCTION MOTOR=%.4f WATTS \n',ILst);
fprintf('\n THE IRON LOSS IN STATOR CORE OF THE INDUCTION MOTOR=%.4f WATTS \n',ILsc);
fprintf('\n THE TOTAL IRON LOSSES IN STATOR OF THE INDUCTION MOTOR=%.4f WATTS \n',Itl);
fprintf('\n THE TOTAL FRICTION AND WINDIAGE LOSSES OF THE INDUCTION MOTOR=%.4f WATTS\n',fwl);
...
fprintf('\n THE PHASE ANGLE OF NO LOAD CURRENT OF THE INDUCTION MOTOR=%.4f DEGREES\n',phaseangle);
fprintf('\n THE EFFICIENCY AT FULL LOAD OF THE INDUCTION MOTOR=%.4f DEGREES\n',eff1); 
fprintf('\n########################################################################## \n');